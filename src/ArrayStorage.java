...
public class ArrayStorage {
    ...
    for (Resume resume : storage) {
		...
		if (resume == null) { // if null element searched in storage null element in storage became "r" and cycle stopped
			storage[i] = r;
			break;
			}
		i++;
    ...
    void delete(String uuid) {
    	...
    	for (Resume resume : storage) {
    		if (resume!=null) {
    			...
    			if (findUuidOk) storage[i] = resumeTemp[i+1];
    		}else break;
    		i++;
    	}
    }
